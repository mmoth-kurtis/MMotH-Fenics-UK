...
def fenics(sim_params,file_inputs,output_params,passive_params,hs_params,cell_ion_params,monodomain_params,windkessel_params):
    ...
    if sim_params["sim_geometry"][0] == "ventricle" or sim_params["sim_geometry"][0] == "ventricle_lclee_2":
    ...
    #s0 = project(s0/sqrt(inner(s0,s0)), VectorFunctionSpace(mesh, "CG", 1))
    #f.read(n00, casename+"/"+"eN")
    #n0 = project(n00, VectorFunctionSpace(mesh, "CG", 1))
    #n0 = project(n0/sqrt(inner(n0,n0)), VectorFunctionSpace(mesh, "CG", 1))
    ...
    #    svec = np.array([s0.vector().array()[p[0]], s0.vector().array()[p[1]], s0.vector().array()[p[2]]])
    #    normsvec = svec/np.linalg.norm(svec)
    #    nvec = np.array([n0.vector().array()[p[0]], n0.vector().array()[p[1]], n0.vector().array()[p[2]]])
    #    normnvec = nvec/np.linalg.norm(nvec)
    #    for k in range(0,3):
    #    	f0.vector()[p[k]] = normfvec[k]
    #    	s0.vector()[p[k]] = normsvec[k]
    #    	n0.vector()[p[k]] = normnvec[k]
