...
def fenics(sim_params,file_inputs,output_params,passive_params,hs_params,cell_ion_params,monodomain_params,windkessel_params):
    ...
    if sim_params["sim_geometry"][0] == "ventricle" or sim_params["sim_geometry"][0] == "ventricle_lclee_2" or sim_params["sim_geometry"][0] == "ventricle_lclee_hstress":
    ...
    #scalepoint = point(0.0,0.0,0.0)
    scalepoint = Point(np.array([0,0,0],dtype='d'))
    MeshTransformation.rescale(mesh,.01,scalepoint)
    ...
    Pactive = Scalefactor * cb_force2 * as_tensor(f0[i]*f0[j], (i,j)) #+ 0.25*cb_force2 * as_tensor(s0CG[i]*s0CG[j], (i,j))+ 0.25*cb_force2 * as_tensor(n0[i]*n0[j], (i,j))
    #Pactive, cb_force = uflforms.TempActiveStress(af_time.af_time)
    #Pactive = Scalefactor * cb_force2 * as_tensor(n0[i]*n0[j], (i,j))
    ...
    Kpen = Constant(100000)
    F6 = Kpen*inner(dot(u,caxis)*caxis, v)*ds(topid)


    Ftotal = F1 + F2 + F3 + F4 # +F4 #F4 #+ F5#F6 + F4 #+ F5 + F4
    ...
    Jac = Jac1 + Jac2 + Jac3 + Jac4 # + Jac6 # +Jac4 #+ Jac6 #Jac4 #+ Jac5 #Jac6 + Jac4#+ Jac5 + Jac4
    ##################################################################################################################################
    ...
    print("cavity-vol in m^-3 = ", LVCavityvol.vol)

    displacementfile = File(output_path + "u_disp.pvd")
    stressfile = File(output_path + "cbforce.pvd")
    pk1file = File(output_path + "pk1_act_on_f0.pvd")
    #pbulkfile = File(output_path + "pbulk.pvd")
    ...
    Pcav_array = [uflforms.LVcavitypressure()*0.75]
    ...
    pbulkarray = np.zeros((no_of_time_steps+1,no_of_int_points))
    ...
    Pbulkf, Pff, alpha = uflforms.stress()
    ...
    temp_DG_Bulk = project(Pbulkf, FunctionSpace(mesh, "DG", 1), form_compiler_parameters={"representation":"uflacs"})
    pbulk = interpolate(temp_DG_Bulk, Quad)
    pbulk_array = pbulk.vector().get_local()[:]
...
#        print "scale factor is " + str(scalefactor.a)
        ...
        LVCavityvol.vol += 0.000000004 #LCL change to smaller value
        ...
        stresstemp = project(cb_force2,FunctionSpace(mesh,'DG',0))
        stresstemp.rename("cbforce","cbforce")
        ...
        if(MPI.rank(comm) == 0):
            ...
            print >>fdataPV, 0.0, p_cav , 0.0, 0.0, V_cav, 0.0, 0.0, 0.0
            displacementfile << w.sub(0)
            stresstemp = project(cb_force2,FunctionSpace(mesh,'DG',0))
            ...
            pk1temp.rename("pk1temp","pk1temp")
            pk1file << pk1temp
            #pbulktemp = project(Pbulkf,FunctionSpace(mesh,'DG',0))
            #pbulktemp.rename("pbulk","pbulk")
            #pbulkfile << pbulk
            hsl_temp = project(hsl,FunctionSpace(mesh,'DG',1))
            hsl_temp.rename("hsl_temp","hsl")
            hsl_file << hsl_temp
    ...
    while(cycle < cycles):
       ...
       # Checking size of things passed if __name__ == '__main__':
        ...
        if(MPI.rank(comm) == 0):
            print >>fdataPV, tstep, p_cav, Part, Pven, V_cav, V_ven, V_art, calcium[counter]
            if save_output:
                print "saving output"
                ...
                np.save(output_path + "pbulkarray", pbulkarray)
                #np.save(output_path + "dumped_populations",dumped_populations)
...
###########################################################################
	...
	print max(cb_f_array), min(cb_f_array)
        ...
        pbulkarray[counter,:] = pbulk_array
