...
def fenics(sim_params,file_inputs,output_params,passive_params,hs_params,cell_ion_params,monodomain_params,windkessel_params):
    ...
    if sim_params["sim_geometry"][0] == "ventricle" or sim_params["sim_geometry"][0] == "ventricle_lclee_2" or sim_params["sim_geometry"][0] == "ventricle_circbc":
    ...
    #n0CG = project(n0, VectorFunctionSpace(mesh, "CG", 1))
    ...
    #---------------------------------------------------------------------------
    #------------ Kurtis 5/27 --------------------------------------------------
    # LC Lee thinks passive parameters play a big role, and constraining
    # circumferential at base only does not prevent rigid body motion.
    # 1) Re-run physio rat params with correct C2.
    # 2) Run the code LCL sent to see what his PV loop looked like (is it
    #    physiological?)
    # 3) Run just the circulatory bc's with his passive params
    # 4) Experiment with rigid body constrain on body, circumferential at base,
    #    does it overconstrain?
    #---------------------------------------------------------------------------
    #---------------------------------------------------------------------------
    ...
    Jac = Jac1 + Jac2 + Jac3 + Jac6 #+ Jac5 + Jac4
    ##################################################################################################################################
